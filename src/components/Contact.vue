...
const emailUnderlineLatest = ref<HTMLDivElement | null>(null);
...
const copyBtnEnter = ()=>{
  ...
  emailUnderlineLatest.value = clone;
  clone.style.left='0%';
  emailUnderline.value.parentNode.appendChild(emailUnderlineLatest.value);
  clone.animate(
    [
      {left: '-100%'},
      {left: '0%'}
...
const copyBtnLeave = ()=>{
  const cur=emailUnderlineLatest.value
  if(!cur) return;
  cur.animate(
    [
      {left: '100%'}
...
<template>
  ...
  <div class="mt-40 mb-20 flex flex-col justify-center">
    <div class="font-marcellus tracking-[0.2em] text-7xl text-center py-4">CONTACT</div>
